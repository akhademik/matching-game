export const getRandomItems = (items: string[], _half_items_length: number) => {
...
export const shuffleItems = (items: string[]) => {
    for (let last_item_index = items.length - 1; last_item_index > 0; last_item_index--) {
        ...
        [items[last_item_index], items[random_index]] = [
            items[random_index],
            items[last_item_index]
        ];
    }
    return items;
};
